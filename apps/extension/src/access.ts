...
function registrationFor(
  ...
  return {
    ...
    // First-store contract: the active top document only. Frame expansion needs
    // its own permission and reveal-policy review.
    allFrames: false,
...
function sameStrings(left: readonly string[] | undefined, right: readonly string[]) {
  ...
  return [...left].sort().every((value, index) => value === right[index]);
}
...
function registrationConverged(
  ...
  return (
    ...
    sameStrings(existing.css, ['content.css']) &&
...
async function convergeRegistration() {
  ...
  for (let attempt = 0; attempt < REGISTRATION_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      // Permission events, popup grants, and startup can race. Treat duplicate or
      // already-removed failures as a cue to reread Chrome's current state.
      lastError = error;
    }
...
export async function removeHostAccess(origins: readonly string[]) {
  ...
  // URL-filtered tabs.query is available while this host permission is held.
  const affectedTabs = await chrome.tabs.query({ url: patterns });
  const tabIds = affectedTabs.flatMap(tab =>
    tab.id === undefined ? [] : [tab.id]
  );

  // Restore exact source while page messaging still has host access.
  ...
  // A narrower remaining grant can still cover some of the restored tabs.
  await Promise.all(tabIds.map(tabId => reactivateTabIfStillGranted(tabId)));
  return true;
}
