...
Arbitrary-page extension reveal must preserve the host page's native tab order. Generated censor roots never receive `tabindex`; click reveal ignores roots nested inside links, buttons, inputs, or similar native controls. The extension's page-wide command is the keyboard reveal path. A reusable renderer may still expose focus reveal when the application owns the rendered control and can provide an intentional focus path. Legacy extension `focus` settings migrate to `click`.

The content runtime starts at `document_start` and each active Scrawlix session belongs to one concrete `document.body`. Begin as soon as `<body>` exists instead of waiting for `DOMContentLoaded`. If a long-running app replaces `<body>` or the document element, disconnect/restore the detached session and start against the new body. Extension-decorated roots carry `data-scrawlix-extension-owned`; cloned or moved copies in a replacement body may be unwrapped to their exact `textContent` before reprocessing, while author-owned lookalike roots remain untouched. Keep permanent lifecycle observation narrow: direct document / `<html>` child changes only. Ordinary subtree mutation work stays with the body-scoped DOM adapter; do not add a second permanent whole-document subtree observer.

The first store-facing coverage contract is top-document-only. Dynamic registration keeps `allFrames: false` and `matchOriginAsFallback: false`; the extension does not traverse open or closed shadow roots. Expanding into child frames, initiator-related `about:`/`blob:`/`data:` frames, or shadow DOM requires an explicit browser-product review, permission/status/reveal semantics, and dedicated restoration/performance fixtures. Closed-shadow traversal is an especially deliberate escalation even though Chrome exposes it to extensions. Track that work under #122 instead of broadening page reach incidentally.
